...
def _run_full_cost(overrides, base_e, base_c_design, base_p_design, base_inputs):
    ...
    try:
        ...
        # The anode is made bigger than the cathode on every edge so the cathode
        # never sits over bare separator, which is what causes plating. So the
        # extra goes on the length as well as the width. Same convention as the
        # anode foil area in run_cell_design.
        _exc = c2.get("anode_excess_mm", 0)
        negative_electrode_area_cm2_2 = (_n_layers * 2
                                         * (c2["electrode_width_mm"] + _exc)
                                         * (c2["electrode_length_mm"] + _exc) / 100)
        r4_2 = run_cost_model(
        ...
        return {
            ...
            "cost_carbon":           r4_2["cost_carbon"],
            "cost_pos_solvent":      r4_2["cost_pos_solvent"],
            "cost_neg_solvent":      r4_2["cost_neg_solvent"],
...
CHEMISTRY_PRESETS_GLOBAL = {
    ...
    # Anode density: 1.50 g/cm3 (Vaalma et al. 2023)
    ...
    # Cathode density: 3.68 g/cm3 (Materials Project mp-19017)
    ...
    # Cycle life: 8000 cycles (Jasper et al. 2026 LFP reference system)
    ...
    "LIB LFP / Graphite": {
        ...
        # Graphite anode uses CMC/SBR binder (not PVDF used for HC anodes)
        # Density and price must override base study HC values
        "a_bind_dens": 1.265,  # CMC/SBR density (BatPaC v5.2 default)
        "p_cmcsbr":    1.4,  # CMC/SBR price for graphite anode (Peters et al. 2016)
...
def _chem_sustainability(r, params, base_in, s5_over=None):
    ...
    try:
        return run_sustainability(
            ...
            # Indexed, not .get(). If one of these ever goes missing we want a
            # loud error, not a quiet zero that drops the jacket and cooling CO2.
            coolant_panel_kg=r["coolant_panel_kg"],
            coolant_manifold_kg=r["coolant_manifold_kg"],
            coolant_liquid_kg=r["coolant_liquid_kg"],
            jacket_support_frame_kg=r["jacket_support_frame_kg"],
            jacket_interior_base_kg=r["jacket_interior_base_kg"],
            jacket_exterior_base_kg=r["jacket_exterior_base_kg"],
            jacket_top_plates_kg=r["jacket_top_plates_kg"],
            module_al_kg=r["module_al_kg"], module_steel_kg=r["module_steel_kg"],
            module_cu_kg=r["module_cu_kg"],
            module_polymer_kg=r["module_polymer_kg"],
...
# - MODULE 04: COST MODEL -
    ...
    with col_out:
        ...
        if calculate4:
            ...
            # Anode oversize goes on both the width and the length, matching the
            # anode foil area in run_cell_design and BatPaC's own reference case.
            negative_electrode_area_cm2 = (_n_layers_m04 * 2
                * (c.get("electrode_width_mm", 0) + _anode_excess_m04)
                * (c.get("electrode_length_mm", 0) + _anode_excess_m04) / 100)
        ...
        if "_m05_results" not in st.session_state:
            ...
            <div class="output-grid">
                <div class="output-card highlight">
                    ...
                    <div class="output-card-value">${r5["lcos_per_kwh"]:.4f}</div>
                    <div class="output-card-unit">$/kWh delivered</div>
                </div>
                <div class="output-card highlight">
                    <div class="output-card-label">LCOS (net, with EOL credit)</div>
                    <div class="output-card-value">${r5["lcos_net_per_kwh"]:.4f}</div>
                    <div class="output-card-unit">$/kWh delivered</div>
                </div>
                <div class="output-card">
                    <div class="output-card-label">Capital component</div>
                    <div class="output-card-value">${r5["lcos_capex_component"]:.4f}</div>
                    <div class="output-card-unit">$/kWh</div>
                </div>
                <div class="output-card">
                    <div class="output-card-label">O&M component</div>
                    <div class="output-card-value">${r5["lcos_om_component"]:.4f}</div>
                    <div class="output-card-unit">$/kWh</div>
                </div>
                <div class="output-card">
                    <div class="output-card-label">Charging energy component</div>
                    <div class="output-card-value">${r5["lcos_energy_component"]:.4f}</div>
                    <div class="output-card-unit">$/kWh</div>
                </div>
                <div class="output-card">
                    <div class="output-card-label">EOL recycling credit</div>
                    <div class="output-card-value">${r5["lcos_eol_credit"]:.4f}</div>
                ...
                <div class="output-card">
                    ...
                    <div class="output-card-value">${r5["cost_per_cycle"]:.4f}</div>
                    <div class="output-card-unit">$/cycle</div>
                </div>
                <div class="output-card">
                    <div class="output-card-label">Annual throughput</div>
                    <div class="output-card-value">{r5["energy_per_year_kwh"]:.4f}</div>
...
# ── MODULE 06: SENSITIVITY ANALYSIS ──────────────────────────────────────────
    ...
    with _tab3:
        ...
        def _render_chem_results(cr):
            ...
            # Split the same way BatPaC reports it: G634 cells, G635 modules,
            # G636 pack hardware, G637 profit, G638 warranty. These add up to the
            # pack total on their own, so nothing has to be plugged in to balance.
            # The cell tier is split again into what the materials cost and what
            # it cost to turn them into a cell.
            _stk_mat=[]; _stk_mfg=[]; _stk_mod=[]; _stk_pw=[]; _stk_hw=[]
            for name in names:
                _r     = cr[name]
                _gross = _r.get("pack_gross_energy", 0)
                if not _gross:
                    _gross = _r.get("pack_useable_energy", 0) / (_useable_soc or 0.84)
                _tc    = _r.get("total_cells", 0)
                _mat   = _r.get("mat_cost_per_cell", 0) * _tc / _gross
                _cells = _r.get("cost_cells_per_pack", 0) / _gross
                _mfg   = _cells - _mat
                _mod   = _r.get("cost_modules_per_pack", 0) / _gross
                ...
                _stk_mod.append(round(_mod,2));  _stk_pw.append(round(_pw,2))
                _stk_hw.append(round(_hw,2))

            _SCOLS = ["#f59e0b","#3b82f6","#8b5cf6","#10b981","#ef4444"]
            fig_stk = go.Figure()
            for _lbl, _vals, _col in [
                ("Cell materials",        _stk_mat,  _SCOLS[0]),
                ("Cell manufacturing",     _stk_mfg,  _SCOLS[1]),
                ("Module hardware",        _stk_mod,  _SCOLS[2]),
        ...
        if "_chem_results" in st.session_state:
            ...
            for _cn in _ordered_names:
                ...
                with _c2:
                    ...
                    # Conductive carbon and the solvent used to mix the binders.
                    # These are materials. Leaving them out pushed them into the
                    # manufacturing slice, which overstated it by about $2.22/kWh.
                    _carb    = _to_kwh(_crd.get("cost_carbon", 0) * _tc2)
                    _solv    = _to_kwh((_crd.get("cost_pos_solvent",0)
                                        + _crd.get("cost_neg_solvent",0)) * _tc2)

                    # Total cell materials in $/kWh
                    _mat_kwh = _cat_am+_an_am+_sep+_elec+_binders+_foils+_cont+_carb+_solv
